...
static func create_sequence(node: Node) -> HoneySequenceSystem:
	return HoneySequenceSystem.new(node)
	

class HoneySequenceSystem:
	...
	func append(animation: HoneyParameters) -> HoneySequenceSystem:
	...
	func append_callback(target: Object,args: Array = [])  -> HoneySequenceSystem:
		var func_ref_animation := HoneyFuncRef.new(target, args)
		return append(func_ref_animation)


	func append_interval(duration: float) -> HoneySequenceSystem:
		var interval_animation := HoneyInterval.new(duration)
		return append(interval_animation)


	func join(animation: HoneyAnimation) -> HoneySequenceSystem:
	...
	func insert(at_position: int,animation: HoneyAnimation) -> HoneySequenceSystem:
